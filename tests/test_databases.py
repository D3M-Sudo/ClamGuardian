...
class FakeProvider:
    def __init__(self, content=b'good', fail=False): self.content, self.fail = content, fail
...
def source():
    return DatabaseSource('thirdparty', 'Third Party', 'Test source',
        (DatabaseArtifact('thirdparty.cvd', 'https://example.invalid/db.cvd', hashlib.sha256(b'good').hexdigest()),))
...
async def test_failed_update_preserves_previous(tmp_path: Path):
    ...
    m.add_source(source()); await m.update('thirdparty')
    m.provider = FakeProvider(b'bad', fail=True)
    with pytest.raises(DatabaseUpdateError): await m.update('thirdparty')
    assert (tmp_path/'db'/'thirdparty.cvd').read_bytes() == b'good'
    assert m.status('thirdparty').status == DatabaseStatus.ERROR

def test_rejects_unsafe_source():
    with pytest.raises(ValueError):
        DatabaseSource('x', 'X', 'x', (DatabaseArtifact('../evil.cvd', 'https://example.invalid/x'),))
